...
{
    setWindowTitle("工程配置 - " + QFileInfo(cmakePath).absolutePath());
...
void ProjectConfigDialog::setupUI()
{
    ...
    QGroupBox *pathGroup = new QGroupBox("SDK路径设置");
    QGridLayout *pathLayout = new QGridLayout(pathGroup);
    pathLayout->addWidget(new QLabel("SDK 路径:"), 0, 0);
    m_sdkPathEdit = new QLineEdit(m_sdkPath);
    m_sdkPathEdit->setReadOnly(true);
    pathLayout->addWidget(m_sdkPathEdit, 0, 1);
    QPushButton *btnBrowseSdk = new QPushButton("浏览...");
    btnBrowseSdk->setMaximumWidth(72);
    ...
    connect(btnBrowseSdk, &QPushButton::clicked, [this]() {
        QString dir = QFileDialog::getExistingDirectory(this, "选择 SDK 目录 (.pico-sdk)", m_sdkPathEdit->text());
        if (!dir.isEmpty()) m_sdkPathEdit->setText(QDir::toNativeSeparators(dir));
    });
    pathLayout->addWidget(btnBrowseSdk, 0, 2);

    QLabel *verLabel = new QLabel("SDK 版本:");
    ...
    QGroupBox *nameGroup = new QGroupBox("工程名称");
    ...
    QGroupBox *sourceGroup = new QGroupBox("源文件");
    ...
    QPushButton *btnAdd = new QPushButton("添加 .c/.cpp");
    ...
    QGroupBox *libGroup = new QGroupBox("链接库");
    ...
    m_stdioUsbCheck = new QCheckBox("USB 串口输出");
    m_stdioUartCheck = new QCheckBox("UART 串口输出");
    m_extraOutputsCheck = new QCheckBox("生成额外输出文件 (.map/.bin/.hex)");
    ...
    QGroupBox *tcGroup = new QGroupBox("工具链");
    ...
    tcLayout->addWidget(new QLabel("编译器类型:"));
    ...
    QGroupBox *stdGroup = new QGroupBox("语言标准");
    QHBoxLayout *stdLayout = new QHBoxLayout(stdGroup);
    stdLayout->addWidget(new QLabel("C 标准:"));
...
void ProjectConfigDialog::onAddSource()
{
    ...
    if (files.isEmpty()) {
        QMessageBox::information(this, "提示", "未找到可添加的源文件。\n请在工程目录下放置 .c/.cpp/.S 文件。");
        return;
    }
...
void ProjectConfigDialog::onSave()
{
    ...
    if (!file.open(QIODevice::WriteOnly | QIODevice::Text | QIODevice::Truncate)) {
        QMessageBox::warning(this, "错误", "无法写入 CMakeLists.txt:\n" + file.errorString());
        return;
    }
