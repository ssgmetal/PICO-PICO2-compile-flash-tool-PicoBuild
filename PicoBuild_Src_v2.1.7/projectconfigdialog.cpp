...
void ProjectConfigDialog::setupUI()
{
    ...
    // ---- Chip/Architecture flags: each bit = one specific chip+arch combo ----
    constexpr uint8_t C_RP2040       = 1 << 0;  // RP2040 (ARM Cortex-M0+)
    constexpr uint8_t C_RP2350_ARM   = 1 << 1;  // RP2350 ARM (Cortex-M33/M23)
    constexpr uint8_t C_RP2350_RISCV = 1 << 2;  // RP2350 RISC-V (Hazard3)

    constexpr uint8_t C_ALL    = C_RP2040 | C_RP2350_ARM | C_RP2350_RISCV;
    constexpr uint8_t C_RP2350 = C_RP2350_ARM | C_RP2350_RISCV;
    constexpr uint8_t C_ARM    = C_RP2040 | C_RP2350_ARM;

    // Auto-generate platform tag from flags
    auto chipTag = [](uint8_t f) -> QString {
        if (f == C_ALL) return {};
        bool a = f & C_RP2040, b = f & C_RP2350_ARM, c = f & C_RP2350_RISCV;
        if (a && !b && !c) return QStringLiteral(" [RP2040]");
        if (!a && b && c)  return QStringLiteral(" [RP2350]");
        if (!a && b && !c) return QStringLiteral(" [RP2350/ARM]");
        if (!a && !b && c) return QStringLiteral(" [RP2350/RISC-V]");
        if (a && b && !c)  return QStringLiteral(" [ARM]");
        return {};
    };

    // Auto-generate color from flags
    auto chipColor = [](uint8_t f) -> QString {
        if (f == C_ALL || f == C_RP2040) return QStringLiteral("#0078d4");           // blue
        if (f == C_RP2350_RISCV)         return QStringLiteral("#006400");           // green
        if (!(f & C_RP2350_RISCV))       return QStringLiteral("#4a0080");           // purple
        if (f == C_RP2350)               return QStringLiteral("#8B4513");           // brown
        return QStringLiteral("#0078d4");
    };

    struct LibInfo { QString name; QString desc; uint8_t chip; };
    QList<LibInfo> allLibs = {
        {"pico_stdlib",        "pico_stdlib (基础库)",           C_ALL},
        {"hardware_uart",      "hardware_uart (串口)",           C_ALL},
        {"hardware_i2c",       "hardware_i2c (I2C)",             C_ALL},
        {"hardware_spi",       "hardware_spi (SPI)",             C_ALL},
        {"hardware_pwm",       "hardware_pwm (PWM)",             C_ALL},
        {"hardware_adc",       "hardware_adc (ADC)",             C_ALL},
        {"hardware_timer",     "hardware_timer (定时器)",        C_ALL},
        {"hardware_watchdog",  "hardware_watchdog (看门狗)",     C_ALL},
        {"hardware_flash",     "hardware_flash (Flash)",         C_ALL},
        {"hardware_sync",      "hardware_sync (同步)",           C_ALL},
        {"hardware_dma",       "hardware_dma (DMA)",             C_ALL},
        {"hardware_pio",       "hardware_pio (PIO)",             C_ALL},
        {"hardware_clocks",    "hardware_clocks (时钟)",         C_ALL},
        {"hardware_irq",       "hardware_irq (中断)",            C_ALL},
        {"hardware_gpio",      "hardware_gpio (GPIO)",           C_ALL},
        {"hardware_rtc",       "hardware_rtc (RTC)",             C_RP2040},
        {"pico_multicore",     "pico_multicore (多核)",          C_ALL},
        {"hardware_powman",    "hardware_powman (电源管理)",     C_RP2350},
        {"hardware_riscv_platform_timer", "hardware_riscv_platform_timer (平台定时器)", C_RP2350},
        {"pico_sha256",        "pico_sha256 (SHA256库)",         C_RP2350},
        {"hardware_dcp",       "hardware_dcp (数据拷贝)",        C_RP2350_ARM},
        {"hardware_rcp",       "hardware_rcp (协处理器)",        C_RP2350_ARM},
        {"cmsis_core",         "cmsis_core (CMSIS)",             C_ARM},
        {"hardware_riscv",     "hardware_riscv (RISC-V硬件)",    C_RP2350_RISCV},
        {"hardware_hazard3",   "hardware_hazard3 (Hazard3内核)", C_RP2350_RISCV},
        {"pico_aon_timer",     "pico_aon_timer (常开定时器)",    C_ALL},
        {"pico_atomic",        "pico_atomic (原子操作)",         C_ALL},
        {"pico_rand",          "pico_rand (随机数)",             C_ALL},
    };

    // Map platform string to chip flag
    uint8_t curChip = 0;
    if (m_platform == "rp2040")          curChip = C_RP2040;
    else if (m_platform == "rp2350-arm-s")  curChip = C_RP2350_ARM;
    else if (m_platform == "rp2350-riscv")  curChip = C_RP2350_RISCV;

    // Filter + build UI
    m_libListWidget = new QListWidget;
    m_libListWidget->setStyleSheet(QStringLiteral(
        "QListWidget { border: 1px solid #c0c0c0; border-radius: 4px; padding: 4px; }"
        "QListWidget::item { padding: 4px 8px; }"
        "QListWidget::item:hover { background-color: #d0d0d0; }"
        "QListWidget::item:selected { background-color: #cce5ff; }"));
    {
        QPalette pal = m_libListWidget->palette();
        pal.setColor(QPalette::HighlightedText, QColor("#1a1a1a"));
        m_libListWidget->setPalette(pal);
    }
    m_libListWidget->setSpacing(2);
    for (const auto &lib : allLibs) {
        if (!(lib.chip & curChip)) continue;
        QString label = lib.desc + chipTag(lib.chip);
        QString color = chipColor(lib.chip);
        QListWidgetItem *item = new QListWidgetItem(label);
        item->setData(Qt::UserRole, lib.name);
        item->setFlags(item->flags() | Qt::ItemIsUserCheckable);
        item->setCheckState(m_libraries.contains(lib.name) ? Qt::Checked : Qt::Unchecked);
        item->setForeground(QColor(color));
        QFont f = item->font();
        f.setBold(true);
        f.setPointSize(10);
        item->setFont(f);
        m_libListWidget->addItem(item);
        m_libMap[lib.name] = item;
    }
    QScrollArea *libScroll = new QScrollArea;
    libScroll->setWidget(m_libListWidget);
    libScroll->setWidgetResizable(true);
    libScroll->setMinimumHeight(200);
    libScroll->setMaximumHeight(400);
    QVBoxLayout *libGroupLayout = new QVBoxLayout(libGroup);
    libGroupLayout->setContentsMargins(4, 4, 4, 4);
    libGroupLayout->addWidget(libScroll);

    topLayout->addWidget(sourceGroup, 1);
    topLayout->addWidget(libGroup, 2);
...
QString ProjectConfigDialog::generateCMakeLists() const
{
    ...
    QString board;
    if (m_platform == "rp2350-arm-s" || m_platform == "rp2350-riscv")
        board = "pico2";
    else
        board = "pico";
    ...
    result += "set(PICO_BOARD " + board + " CACHE STRING \"Board type\")\n\n";
...
static QMap<QString, QStringList> s_libHeaders()
{
    ...
    if (map.isEmpty()) {
        ...
        map["hardware_rtc"]       = {"hardware/rtc.h"};
                                ...
                                const QMap<QString, QListWidgetItem *> &libMap)
{
    ...
    QMap<QString, QListWidgetItem *> libMapRef = libMap;
    ...
    for (auto it = libMapRef.begin(); it != libMapRef.end(); ++it) {
        ...
        for (const QString &hdr : hdrs) {
            ...
            if (it.value()->checkState() == Qt::Checked && !hasInclude) {
                QString newLine = "#include \"" + hdr + "\"";
                lines.insert(lastIncludeIdx + 1, newLine);
                lastIncludeIdx++;
            } else if (it.value()->checkState() != Qt::Checked && hasInclude) {
    ...
    // Clean up stale includes that belong to unchecked libraries or removed mappings
    // Collect all valid headers from currently checked libraries
    QSet<QString> validHeaders;
    for (auto it = libMapRef.begin(); it != libMapRef.end(); ++it) {
        if (it.value()->checkState() == Qt::Checked && headers.contains(it.key())) {
            for (const QString &h : headers[it.key()])
                validHeaders.insert(h);
        }
    }
    // Collect all known auto-include headers (including from unchecked libraries)
    QSet<QString> knownHeaders;
    for (auto hit = headers.begin(); hit != headers.end(); ++hit) {
        for (const QString &h : hit.value())
            knownHeaders.insert(h);
    }
    // Known stale headers from removed mappings
    knownHeaders.insert("pico/sleep.h");
    knownHeaders.insert("hardware/dcp.h");
    knownHeaders.insert("pico/atomic.h");
    knownHeaders.insert("cmsis/arm_math.h");
    knownHeaders.insert("cmsis/core_cm33.h");
    // Remove any auto-include that is known but not in the valid set
    for (int i = lines.size() - 1; i >= 0; --i) {
        QString line = lines[i].trimmed();
        if (!line.startsWith("#include"))
            continue;
        for (const QString &hdr : knownHeaders) {
            QRegularExpression re("#include\\s*[\"<]" + QRegularExpression::escape(hdr) + "[\">]");
            if (re.match(line).hasMatch() && !validHeaders.contains(hdr)) {
                lines.removeAt(i);
            }
        }
    }
